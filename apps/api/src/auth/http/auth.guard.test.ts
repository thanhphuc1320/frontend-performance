...
describe('AuthGuard', () => {
  ...
  function makeSessionService() {
    return {
      validate: jest.fn(async () => null as { userId: string } | null),
  ...
  it('allows safe methods without CSRF check', async () => {
    ...
    service.validate.mockResolvedValueOnce({ userId: 'user-1' });
    const guard = new AuthGuard(service as never);

    const context = makeContext({ method: 'GET', cookies: { commerce_session: 'token' } });
    const result = await guard.canActivate(context);

    expect(result).toBe(true);
    expect(service.validateCsrf).not.toHaveBeenCalled();
  });
  ...
  it('requires CSRF token for mutation methods', async () => {
    ...
    expect(result).toBe(true);
    expect(service.validateCsrf).toHaveBeenCalled();
  });
