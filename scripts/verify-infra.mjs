...
async function postgresChecks() {
  ...
  try {
    ...
    const before = await client.query('SELECT version FROM schema_migrations WHERE version IN ($1, $2, $3) ORDER BY version', ['0001', '0002', '0003']);
    if (before.rowCount !== 3) throw new Error('Auth/Store/RBAC migrations are not recorded exactly once');
    ...
    const after = await client.query('SELECT count(*)::int AS count FROM schema_migrations WHERE version IN ($1, $2, $3)', ['0001', '0002', '0003']);
    ...
    await run('node', ['infra/postgres/migrate.mjs', 'up']);
    const restored = await client.query('SELECT count(*)::int AS count FROM schema_migrations WHERE version IN ($1, $2, $3)', ['0001', '0002', '0003']);
    if (restored.rows[0].count !== 3) throw new Error('Auth/Store/RBAC migrations were not restored');
  } finally {
    if (sentinelInserted) {
